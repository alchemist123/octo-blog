...
- **Email/Password Registration**: Sign up with username and password
- **Email/Password Login**: Login with credentials
- **OAuth Support**: Google, GitHub, and GitLab authentication
...
```
Response includes JWT token in `x-access-token` header.
...
```
Response includes user data (without password) and JWT token in `x-access-token` header.
